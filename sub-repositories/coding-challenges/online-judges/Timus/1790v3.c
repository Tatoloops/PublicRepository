	...
	if (n==2)
		...
		if (count[1]==1){ 
			while(part & 0x2){
				--m;
				scanf("%d",&a);
				if (par & 0x1){ // 1 0 1 0 1  -- 1 is the treasure - 0 is an empty space
					if (a==aux[0]+2 && a&0x1) ++aux[0]; //a impar
					else --aux[0];
					if (a==n-aux[1]-1 && a&0x1) ++aux[1];
					else --aux[1];
				}
				else{ // 1 0 1 0
					if (a==aux[0]+2 && a&0x1) ++aux[0];
					else --aux[0];
					if (a==n-aux[1]-1 && a&0x1) ++aux[1];
					else --aux[1];
				}
